...
public class LaneView implements ActionListener, Observer {
	...
	public void update(Observable lane, Object le) {
		if (((LaneEvent)le).isPartyAssigned) {
			...
			if (((LaneEvent)le).getFrameNum() == 1
				&& ((LaneEvent)le).getBall() == 0
				&& ((LaneEvent)le).getIndex() == 0) {
			...
			for (int k = 0; k < numBowlers; k++) {
				...
				for (int i = 0; i < 21; i++) {
					if (((int[]) ((HashMap) ((LaneEvent)le).getScore())
						.get(bowlers.get(k)))[i]
						!= -1)
						if (((int[]) ((HashMap) ((LaneEvent)le).getScore())
							.get(bowlers.get(k)))[i]
							...
							i > 0
								&& ((int[]) ((HashMap) ((LaneEvent)le).getScore())
									.get(bowlers.get(k)))[i]
									+ ((int[]) ((HashMap) ((LaneEvent)le).getScore())
										.get(bowlers.get(k)))[i
									- 1]
						...
						else if ( ((int[])((HashMap) ((LaneEvent)le).getScore()).get(bowlers.get(k)))[i] == -2 ){
							
							ballLabel[k][i].setText("F");
						} else
							ballLabel[k][i].setText(
								(new Integer(((int[]) ((HashMap) ((LaneEvent)le).getScore())
									.get(bowlers.get(k)))[i]))
									.toString());
				}
			}
